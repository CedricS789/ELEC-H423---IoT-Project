...
extern uint8_t key_encrypt[32];     // passing the key to h file
...
void write_key_flash(){
    static const uint8_t key [32] =   {0xC6, 0x62, 0x20, 0xC5, 0xE7, 0xD9, 0x18, 0x61, 0x30, 0xD1, 0x52, 0xF8, 0x96, 0x19, 0x83, 0x38,
                                       0x96, 0x0B, 0x22, 0x1F, 0x29, 0x77, 0x05, 0x27, 0x7C, 0x22, 0x8B, 0xED, 0x82, 0x99, 0x72, 0x15};
...
void load_key_flash(){
    prefs.begin("Encryption",false);
    prefs.getBytes("Encrypted_AES",key_encrypt,sizeof(key_encrypt));
    prefs.end();
}
...
String aes_encrypt(const String& plaintext){
    ...
    String AES_check = text_encoder(key_plain,16);
    ...
    mbedtls_aes_setkey_enc(&aes, key_plain, 128);
...
String aes_decrypt(const String& ciphertext_b64, uint32_t* out_counter_rx)
{
    ...
    mbedtls_aes_setkey_dec(&aes, key_plain, 128);
...
bool aes_decrypt_key(const String& ciphertext_b64, uint8_t* pbkdf_key, uint8_t* output)
{
    // 1) Base64 decode: ciphertext_b64 -> [IV(16 bytes) || CIPHERTEXT]
    uint8_t* decoded = nullptr;
    size_t decoded_len = 0;
    ...
    // Set up the AES context for decryption with a 128-bit key (16 bytes * 8 = 128 bits).
    mbedtls_aes_setkey_dec(&aes, pbkdf_key, 128);
...
String Hmac_generate(const uint8_t* data, size_t data_len) {
    ...
    mbedtls_md_hmac(md_info, key_plain, sizeof(key_plain), data, data_len, hmac_output);    // Hashed generated - 32 bytes
